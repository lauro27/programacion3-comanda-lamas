...
class Pedido
{
    ...
    public function modificarPedido()
    {
        ...
        //si existe
        if($this->estimado){
            //si no fue formateado
            if(gettype($this->estimado) != "string"){
                $est = $this->estimado->format('Y-m-d H:i:s');
            }
           else {$est = $this->estimado;}
        }
        else{$est = null;}
        $consulta->bindValue(':estimado', $est);
        
        if($this->hora_entrega){
            if(gettype($this->hora_entrega) != "string"){
                $ent = $this->hora_entrega->format('Y-m-d H:i:s');
            }
           else {$ent = $this->hora_entrega;}
        }
        else{$ent = null;}
    ...
    public static function TraerTodosMesa(string $codigo)
    {
        $cancelado = "cancelado";
        $pagado = "pagado";
        $objAccesoDatos = AccesoDatos::obtenerInstancia();
        $consulta = $objAccesoDatos->prepararConsulta("SELECT * FROM pedidos 
                WHERE cod_mesa = :cod_mesa and 
                    not estado = :estadoCancelado and 
                    not estado = :estadoPagado");
        $consulta->bindValue(":cod_mesa", $codigo);
        $consulta->bindValue(":estadoCancelado", $cancelado);
        $consulta->bindValue(":estadoPagado", $pagado);
